...
/* ---------------- coaching entitlement (demonstration only) ----------------
   Deliberately client-side. There is no payment in this product: companies buy
   credits by bank transfer, and nothing here takes money from a student. This
   exists to show WHERE a paid tier would sit, not to implement one. A real
   entitlement would be a server-side record, checked by the API, and no button
   in a browser could grant it. */
const coaching = {
  key(){ return "wasta.coaching." + (session.get()?.email || "anon"); },
  get unlocked(){ return localStorage.getItem(this.key()) === "yes"; },
  unlock(){ localStorage.setItem(this.key(), "yes"); },
  lock(){ localStorage.removeItem(this.key()); },
};
...
route(/^\/coach$/, async host => {
  if (!coaching.unlocked) { paywall(host); return; }
  ...
  host.innerHTML = head("Career Coach", "Written from your section scores — and never repeats your score back at you.") +
    `<div class="card" style="background:var(--brand-wash);border-color:var(--brand-200)">
     ...
     <p class="tiny muted"><a href="#" id="relock">Lock this again</a> — resets the demonstration.</p>`;

  $("#relock").onclick = e => { e.preventDefault(); coaching.lock(); render(); };
}, isSeeker);

/**
 * The upsell. Shows what the plan contains without giving it away, and takes
 * no payment details of any kind - a single button, and a line saying so.
 */
function paywall(host) {
  host.innerHTML = head("Career Coach", "A four-week study plan written from your own section scores.") +
    `<div class="card" style="background:var(--brand-wash);border-color:var(--brand-200)">
       <div class="between">
         <div>
           <span class="pill pill-accent">Wasta Coaching</span>
           <h2 style="margin-top:10px">Turn your weakest sections into a plan</h2>
         </div>
         <div class="right">
           <div style="font-size:34px;font-weight:750;color:var(--brand-ink);line-height:1">EGP 149</div>
           <div class="tiny muted">one payment · per assessment</div>
         </div>
       </div>
       <ul class="plain" style="margin-top:16px">
         <li class="small">✦ A written read of where you actually stand, section by section</li>
         <li class="small">✦ Four weeks of concrete work, with a checkpoint you can test yourself against</li>
         <li class="small">✦ A project to build, chosen to cover the gaps the assessment found</li>
         <li class="small">✦ A line you can use in an interview about how you closed them</li>
       </ul>
       <div class="row" style="margin-top:18px">
         <button class="btn" id="buy">Unlock coaching</button>
         <span class="tiny muted">No payment is taken. This is a demonstration of where a paid tier would sit.</span>
       </div>
     </div>
     <div class="card">
       <h3>Your score and your place in the talent pool stay free</h3>
       <p class="small muted" style="margin-top:6px">
         Sitting the assessment, receiving your Wasta Score, appearing to companies and applying to jobs
         cost a student nothing, and never will. Charging to be seen would rebuild the barrier this
         platform exists to remove. Coaching is optional help on top of that, not the way in.
       </p>
     </div>`;

  $("#buy").onclick = () => { coaching.unlock(); render(); };
}
